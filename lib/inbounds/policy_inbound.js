...
function policyInbound (policy) {
  return async function inbound (resource, attributesArray, actionContext = {}) {
    ...
    async function hasUniqueViolation (attributes, uniqueFilter, {id}) {
      let exists = await resource.exists(uniqueFilter)
      if (!exists) {
        return false
      }
      if (id) {
        let first = await resource.first(uniqueFilter)
    ...
    for (let [index, attributes] of attributesArray.entries()) {
      ...
      for (let uniqueFilter of uniqueFilters) {
        let hasViolation = await hasUniqueViolation(attributes, uniqueFilter, {id})
        if (hasViolation) {
          let error = new PolicyError(`[${resourceName}] Unique constraint violation`, {
            conflict: uniqueFilter,
