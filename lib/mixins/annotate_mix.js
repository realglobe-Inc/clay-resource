...
const { annotationStripFormatter } = require('../formatters')
...
function annotateMix (BaseClass) {
  class AnnotateMixed extends BaseClass {
    ...
    constructor () {
      ...
      const s = this
      if (!s.$$formatMixed) {
        throw new Error('FormatMix is required')
      }
    ...
    annotates (annotates) {
      ...
      if (annotates) {
        s.removeFormatter(ANNOTATE_STRIP)
      } else {
        s.addFormatter(ANNOTATE_STRIP, annotationStripFormatter())
      }
