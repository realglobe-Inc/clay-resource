...
const processForResource = (resource) => (entities) => co(function * () {
  ...
  const parseEntityAttribute = (name, attribute) => co(function * () {
    if (Array.isArray(attribute)) {
      return yield attribute.map((attribute) => parseEntityAttribute(name, attribute))
    }
  ...
  for (let entity of entities) {
    for (let name of Object.keys(entity)) {
      ...
      entity[ name ] = yield parseEntityAttribute(name, attribute)
    }
  }
  return entities
})
