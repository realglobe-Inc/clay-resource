...
function refOutbound (refResource) {
  ...
  async function outboundAttribute (resourceName, name, attribute, actionContext) {
    ...
    if (Array.isArray(attribute)) {
      return Promise.all(
        attribute.map((attribute) => {
          ...
          return outboundAttribute(resourceName, name, attribute, actionContext)
        })
      )
    }
    ...
    if (hit) {
      const refKey = refTo(refResource, $ref.id)
      const resolved = resolvedRefs[refKey]
      if (resolved) {
        return resolved
      }
      const found = refResource.one($ref.id, {
        ignoreCached,
  ...
  return async function outbound (resource, entities, actionContext = {}) {
    ...
    for (const entity of entities) {
      const names = Object.keys(entity).filter((name) => !/^_/.test(name))
