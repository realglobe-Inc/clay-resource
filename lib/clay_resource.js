...
class ClayResource extends ClayResourceBase {
  ...
  async one (id, options = {}) {
    ...
    const one = await this.bounds.one(id)
    if (one) {
      this.storeCache(one)
    } else {
      this.removeCache(id)
  ...
  async create (attributes = {}, options = {}) {
    ...
    await this.prepareIfNeeded()
    const actionContext = actionContextFor('create', {errorNamespace, suppressWarning,})
    ...
    const raw = await this.bounds.create(values)
    const created = await this.outboundEntity(raw, actionContext)
    await this.emit(ENTITY_CREATE, {created})
    await tick()
    return created
  }
  ...
  async update (id, attributes = {}, options = {}) {
    ...
    await this.prepareIfNeeded()
    id = this._formatId(id)
    const actionContext = actionContextFor('update', {id, errorNamespace, suppressWarning,})
    attributes = await this.inboundAttributes(attributes, actionContext)
    ...
    const values = asEntity().set(attributes, {allowReserved}).as(this._resourceNameString).toValues()
    const raw = await this.bounds.update(id, values)
    const updated = await this.outboundEntity(raw, actionContext)
    await this.emit(ENTITY_UPDATE, {id, old, updated})
    await this.emit(INVALIDATE, {id})
    await tick()
    return updated
  }
  ...
  async destroy (id) {
    ...
    await this.emit(ENTITY_DESTROY, {id, gone, destroyed})
    await this.emit(INVALIDATE, {id})
    await tick()
    return destroyed
  }
  ...
  async drop () {
    ...
    await this.emit(ENTITY_DROP, {dropped})
    await tick()
    return dropped
  }
  ...
  async oneBulk (ids) {
    ...
    const oneHash = await this.bounds.oneBulk(ids)
  ...
  async createBulk (attributesArray = [], options = {}) {
    ...
    const valuesArray = attributesArray.map((attributes) => asEntity().set(attributes).as(this._resourceNameString).toValues())
    const raw = await this.bounds.createBulk(valuesArray)
    const created = await this.outboundEntityArray(raw, actionContext)
    await this.emit(ENTITY_CREATE_BULK, {created})
    await tick()
    return created
  }
  ...
  async updateBulk (attributesHash = {}, options = {}) {
    ...
    const raw = await this.bounds.updateBulk(valuesHash)
    const updated = await this.outboundEntityHash(raw, actionContext)
    await this.emit(ENTITY_UPDATE_BULK, {ids, updated, old})
    await this.emit(INVALIDATE_BULK, {ids})
    await tick()
    return updated
  }
  ...
  async destroyBulk (ids = []) {
    ...
    await this.emit(ENTITY_DESTROY_BULK, {ids, destroyed, gone})
    await this.emit(INVALIDATE_BULK, {ids})
    await tick()
    return destroyed
  }
  ...
  async each (handler, condition = {}) {
    await this.prepareIfNeeded()
    const cursor = await this.cursor(condition)
    let i = 0
    for (const fetch of cursor) {
  ...
  async first (filter, options = {}) {
    ...
    const {entities} = await this.list({filter, page, sort}, {skipResolvingRefFor})
