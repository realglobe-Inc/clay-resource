...
class ClayResource extends ClayResourceBase {
  ...
  list (condition = {}) {
    ...
    return co(function * () {
      ...
      const collection = yield s.bounds.list(
  ...
  cursor (condition = {}) {
    ...
    return co(function * () {
      ...
      return yield s.bounds.cursor({sort, filter, parser})
    })
  }
   ...
   * @returns {Promise}
   */
  each (handler, condition = {}) {
    const s = this
    return co(function * () {
      yield s.prepareIfNeeded()
      const cursor = yield s.cursor(condition)
  ...
  only (filter, options = {}) {
    ...
    return co(function * () {
      ...
      let found = yield s.first(filter)
      if (!found && strict) {
