...
const actionContextFor = (action, values) => Object.assign({}, values, {action})
...
class ClayResource extends ClayResourceBase {
  ...
  async list (condition = {}) {
    ...
    await s.prepareIfNeeded()
    const actionContext = actionContextFor('list', {condition})
  ...
  async create (attributes = {}, options = {}) {
    ...
    let values = asEntity().set(attributes, {allowReserved}).as(s._resourceNameString).toValues()
    let raw = await s.bounds.create(values)
    let created = await s.outboundEntity(raw, actionContext)
  ...
  async update (id, attributes = {}) {
    ...
    attributes = await s.inboundAttributes(attributes, actionContext)
    let has = await s.has(id)
    if (!has) {
      throw s.throwEntityNotFoundError(id)
    }
    let values = asEntity().set(attributes).as(s._resourceNameString).toValues()
    let raw = await s.bounds.update(id, values)
    let updated = await s.outboundEntity(raw, actionContext)
  ...
  async destroy (id) {
    ...
    await s.prepareIfNeeded()
    id = s._formatId(id)
    let destroyed = await s.bounds.destroy(id)
  ...
  async drop () {
    ...
    let dropped = await s.bounds.drop()
  ...
  async oneBulk (ids) {
    ...
    let oneHash = await s.bounds.oneBulk(ids)
  ...
  async createBulk (attributesArray = []) {
    ...
    let valuesArray = attributesArray.map((attributes) => asEntity().set(attributes).as(s._resourceNameString).toValues())
    let raw = await s.bounds.createBulk(valuesArray)
    let created = await s.outboundEntityArray(raw, actionContext)
  ...
  async updateBulk (attributesHash = {}) {
    ...
    let valuesHash = ids.reduce((valuesHash, id) => Object.assign(valuesHash, {
      [id]: asEntity().set(attributesHash[id]).as(s._resourceNameString).toValues()
    }), {})
    let raw = await s.bounds.updateBulk(valuesHash)
    let updated = await s.outboundEntityHash(raw, actionContext)
  ...
  async destroyBulk (ids = []) {
    ...
    await s.prepareIfNeeded()
    ids = ids.map(sureId)
    let destroyed = await s.bounds.destroyBulk(ids)
  ...
  async first (filter, options = {}) {
    ...
    let {entities} = await s.list({filter, page, sort})
  ...
  async only (filter, options = {}) {
    ...
    await s.prepareIfNeeded()
    let count = await s.count(filter)
    if (count > 1) {
      throw SituationError(`Multiple entity found for only entity`, {
        count,
        filter
      })
    }
    let found = await s.first(filter)
    if (!found && strict) {
  ...
  async seal (privateKey, options = {}) {
    ...
    await s.prepareIfNeeded()
    let cursor = await s.cursor({
      // TODO Filter with seal
    })
    for (let fetch of cursor) {
      let fetched = await fetch()
      let {id} = fetched
      let {$$seal, $$by} = asEntity(fetched)
        .by(by)
  ...
  async has (id) {
    ...
    await s.prepareIfNeeded()
    id = s._formatId(id)
    let one = await s.one(id, {strict: false})
    return !!one
  }
  ...
  async count (filter = {}) {
    ...
    await s.prepareIfNeeded()
    let {meta} = await s.list({page: {number: 1, size: 1}, filter})
    return meta.total
  }
  ...
  async of (attributes = {}) {
    ...
    await s.prepareIfNeeded()
    let found = await s.first(attributes)
    if (found) {
      return found
    }
  ...
  async all (filter, options = {}) {
    ...
    await s.prepareIfNeeded()
    let {entities} = await s.list({filter, sort})
